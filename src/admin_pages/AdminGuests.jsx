...
export default function AdminGuestsPage() {
  ...
  const loadGuests = useCallback(async () => {
    try {
      ...
      if (blacklistFilter !== "all") params.is_blacklisted = blacklistFilter === "blacklisted";
  ...
  const openGuestDetail = async (guest) => {
    ...
    try {
      ...
      setSelectedGuestReservations(Array.isArray(reservations) ? reservations : []);
    } catch {
      setSelectedGuestReservations([]);
    }
  };
  ...
  return (
    <>
      ...
      <div data-component="AdminGuests" className="px-[4rem] max-sm:px-[1rem] py-[4rem] flex flex-col items-start gap-[3rem]">
              ...
              {isFilterOpen && (
                <div className="absolute right-0 mt-2 w-96 bg-white border border-[color:var(--text-color)]/15 rounded-xl shadow-xl z-20 overflow-hidden font-primary">
                  <div className="p-6 border-b border-[color:var(--text-color)]/10">
                    <p className="text-lg font-bold text-[color:var(--text-color)]/70 uppercase tracking-widest mb-4">Guest Type</p>
                    <div className="grid grid-cols-2 gap-3">
                      {["all", ...GUEST_TYPES].map((t) => (
                        <button
                          key={t}
                          onClick={() => { setGuestTypeFilter(t); setIsFilterOpen(false); }}
                  ...
                  <div className="p-6">
                    <p className="text-lg font-bold text-[color:var(--text-color)]/70 uppercase tracking-widest mb-4">Blacklist Status</p>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { key: "all", label: "All" },
                        ...
                        <button
                          key={opt.key}
                          onClick={() => { setBlacklistFilter(opt.key); setIsFilterOpen(false); }}
            ...
            <table className={table.el}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan="6" className="px-8 py-10 text-center text-xl text-[color:var(--text-color)]/50">No guests match filter.</td></tr>
                ) : (
                  guests.map((g) => (
                    <tr key={g.id} className={table.row}>
                      ...
                      <td className={`${table.td} hidden md:table-cell`}>{g.email || "N/A"}</td>
                      <td className={`${table.td} hidden md:table-cell`}>{g.phone || "N/A"}</td>
                      <td className={`${table.td} hidden md:table-cell capitalize`}>{g.guest_type || "N/A"}</td>
                      <td className={`${table.td} hidden md:table-cell`}>{g.total_stays}</td>
                      <td className={table.td}>
                        <div className={table.actions}>
                          <button onClick={() => openGuestDetail(g)} className={btn.rowPrimary}>View</button>
                        </div>
                      </td>
                    </tr>
                  ))
      ...
      {selectedGuest && editForm && (
        <Modal
          ...
          badge={selectedGuest.is_blacklisted && (
            <span className="inline-block px-3 py-1 rounded-full text-lg font-bold leading-tight whitespace-nowrap bg-red-100 text-red-700">Blacklisted</span>
          )}
          size="lg"
          footer={
            <>
              <button onClick={closeGuestDetail} className={btn.secondary}>Close</button>
        ...
        >
          <section className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
            <LabeledInput label="First Name" value={editForm.first_name || ""} onChange={(v) => setEditForm({ ...editForm, first_name: v })} />
            <LabeledInput label="Last Name" value={editForm.last_name || ""} onChange={(v) => setEditForm({ ...editForm, last_name: v })} />
            ...
            <LabeledInput label="Address" value={editForm.address || ""} onChange={(v) => setEditForm({ ...editForm, address: v })} />
            <LabeledInput label="City" value={editForm.city || ""} onChange={(v) => setEditForm({ ...editForm, city: v })} />
            <LabeledInput label="Country" value={editForm.country || ""} onChange={(v) => setEditForm({ ...editForm, country: v })} />
            <div className="flex flex-col gap-2">
              <label className={field.label}>Guest Type</label>
              <select
                value={editForm.guest_type || ""}
                onChange={(e) => setEditForm({ ...editForm, guest_type: e.target.value })}
                className={field.select}
              >
                <option value="">—</option>
                {GUEST_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
          </section>
          ...
          <section className="flex flex-col gap-3 border-t border-[color:var(--text-color)]/10 pt-6">
            <h3 className="text-2xl font-bold text-[color:var(--black)]">Reservations</h3>
            {selectedGuestReservations.length === 0 ? (
              <p className="text-xl text-[color:var(--text-color)]/60">No reservations for this guest yet.</p>
            ) : (
              <div className="flex flex-col gap-2">
                {selectedGuestReservations.map((r) => (
                  <div key={r.id} className="flex justify-between items-center gap-4 bg-[color:var(--text-color)]/3 rounded-lg px-5 py-3 text-xl">
                    <span className="truncate">
                      {r.booking_reference || r.id}
                      <span className="text-[color:var(--text-color)]/50 ml-3">
      ...
      {isCreateOpen && (
          ...
          footer={
            <>
              ...
              <button
                ...
                disabled={creating || !createForm.first_name || !createForm.last_name || !createForm.phone}
        ...
        >
          <section className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
            <LabeledInput label="First Name *" value={createForm.first_name} onChange={(v) => setCreateForm({ ...createForm, first_name: v })} />
            <LabeledInput label="Last Name *" value={createForm.last_name} onChange={(v) => setCreateForm({ ...createForm, last_name: v })} />
            ...
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                onChange={(e) => setCreateForm({ ...createForm, guest_type: e.target.value })}
                className={field.select}
              >
                <option value="">—</option>
                {GUEST_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
            ...
            <LabeledInput label="Company Name" value={createForm.company_name} onChange={(v) => setCreateForm({ ...createForm, company_name: v })} />
          </section>
        </Modal>
      )}
    </>
